...
-- CREATE OR REPLACE FUNCTION public.make_like(user_id_ integer, tweet_id_ integer) RETURNS VOID AS
-- $$
-- BEGIN
--     select * from public.liked_tweet where user_id = user_id_ and tweet_id = tweet_id_
--     update public.tweet t set likes = t.likes + 1 where t.tweet_id = tweet_id_;
--     INSERT INTO public.liked_tweet (user_id, tweet_id) VALUES (user_id_, tweet_id_);
-- END;
-- $$ LANGUAGE 'plpgsql';

-- 1
select public."create_tweet_with_media"(1, 'new tweet 1', 'https://www.planetware.com/wpimages/2020/02/france-in-pictures-beautiful-places-to-photograph-eiffel-tower.jpg');
-- 2
select public."create_tweet_with_media"(2, 'new tweet 2', 'https://www.planetware.com/wpimages/2020/02/france-in-pictures-beautiful-places-to-photograph-eiffel-tower.jpg');
-- 3
select public."create_tweet_with_media"(3, 'new tweet 3', 'https://www.planetware.com/wpimages/2020/02/france-in-pictures-beautiful-places-to-photograph-eiffel-tower.jpg');
-- 4
select public."create_tweet_with_media"(3, 'new tweet 3_', 'https://www.planetware.com/wpimages/2020/02/france-in-pictures-beautiful-places-to-photograph-eiffel-tower.jpg');
-- 5
select public."create_tweet_with_media"(2, 'new tweet 2_', 'https://www.planetware.com/wpimages/2020/02/france-in-pictures-beautiful-places-to-photograph-eiffel-tower.jpg');
-- 6
select public."create_tweet_with_media"(1, 'new tweet 1_', 'https://www.planetware.com/wpimages/2020/02/france-in-pictures-beautiful-places-to-photograph-eiffel-tower.jpg');

-- select public."make_like"(1, 1);
-- select public."make_like"(1, 2);
-- select public."make_like"(1, 3);
-- select public."make_like"(2, 1);
-- select public."make_like"(2, 2);
-- select public."make_like"(3, 3);
...
create index user_index on public.user using hash(username);

explain analyze select password from public.user where username = 'aida';

explain analyze select count(t.*) as "tweer_with_media" from public.tweet t left join media m on t.tweet_id = m.tweet_id group by t.tweet_id order by t.tweet_id;



-- select u.username, sum(t.likes) as likes from public.user u inner join public.tweet t on t.user_id = u.user_id group by (u.username);

-- select count(*) from public.liked_tweet where user_id = 1;
